...
/**
 * Get AI-targeted files from repo-intel recent-ai query.
 *
 * Uses the agent-analyzer binary to find files with recent AI-authored changes.
 * Returns file paths sorted by AI change density, or null if unavailable.
 *
 * @param {string} cwd - Repository root path
 * @param {Object} [options]
 * @param {number} [options.limit=50] - Maximum number of files to return
 * @returns {string[]|null} Array of relative file paths, or null on failure
 */
function getAiTargetFiles(cwd, options = {}) {
  const limit = options.limit || 50;

  try {
    // Check if repo-intel.json exists in the state dir
    ...
    // Try to load the binary module from agentsys shared lib
    let binary;
    try {
...
/**
 * Get files with no test coupling from repo-intel test-gaps query.
 *
 * @param {string} cwd - Repository root path
 * @returns {Set<string>|null} Set of file paths with no test coupling, or null on failure
 */
function getTestGaps(cwd) {
  try {
    const stateDir = getStateDir(cwd);
    const mapFile = path.join(cwd, stateDir, 'repo-intel.json');
...
/**
 * Get risk scores for a list of files from repo-intel diff-risk query.
 *
 * @param {string} cwd - Repository root path
 * @param {string[]} files - File paths to score
 * @returns {Map<string, number>|null} Map of path -> riskScore (0-1), or null on failure
 */
function getRiskScores(cwd, files) {
  ...
  try {
    ...
    for (const r of results) scores.set(r.path, r.riskScore);
    return scores;
  } catch {
    return null;
  }
