...
const slopPatterns = {
   ...
   * Only matches debugger imports/calls - print() excluded (legitimate in CLIs)
   */
  python_debugging: {
    pattern: /(?:import pdb|import ipdb|breakpoint\(\)|pdb\.set_trace\(\))/,
    exclude: ['test_*.py', '*_test.py', 'conftest.py'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'python',
    description: 'Debug import/breakpoint left in production (pdb, ipdb, breakpoint())'
  },

  /**
   * Rust: dbg!() macro left in production code
   * dbg!() is always a debugging artifact - never intended for production.
   * Separated from println!/eprintln! which are legitimate CLI output.
   */
  rust_debugging: {
    pattern: /\bdbg!\s*\(/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'rust',
  ...
  /**
   * Rust: println!/eprintln! in library code
   * Legitimate in CLI binaries (main.rs, bin/) and build scripts (build.rs).
   * Flags println!/eprintln! in library source that should use log/tracing.
   */
  rust_println_in_lib: {
    pattern: /(println!|eprintln!)\(/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs', 'build.rs', '**/build.rs', 'src/main.rs', '**/src/main.rs', 'src/bin/**', '**/src/bin/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: 'println!/eprintln! in library code - use log/tracing crate instead'
  },
  ...
  rust_bare_unwrap: {
    pattern: /\.unwrap\(\s*\)(?!\s*\.)/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs', '**/examples/**', '**/benches/**', 'build.rs', '**/build.rs'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: 'Bare .unwrap() can panic - use .expect(), .unwrap_or(), or ? operator. Note: may match doc comment examples (/// lines)'
  },

  /**
   * Rust: log::debug!()/trace!() left in production code
   * These logging macros add noise in production builds
   */
  rust_log_debug: {
    pattern: /\b(?:log|tracing)::(debug|trace)!\s*\(/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: 'debug/trace logging - review if intentional or leftover from debugging'
  },

  /**
   * Rust: Empty error match arm silently swallowing errors
   * Matches Err(e) => {} or Err(_) => () patterns
   */
  rust_empty_match_arm: {
    ...
    autoFix: 'flag',
    language: 'rust',
    description: 'Empty error match arm silently swallowing errors'
  },

  /**
   * Rust: Potential unnecessary .clone() call
   * Flags all .clone() for review - many are avoidable with borrowing
   */
  rust_unnecessary_clone: {
    pattern: /\.clone\(\s*\)/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs', '**/benches/**', 'build.rs', '**/build.rs'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: 'Potential unnecessary .clone() - verify borrowing is not possible'
  },

  /**
   * Rust: unsafe block - verify // SAFETY: comment exists
   * Rust convention requires a // SAFETY: comment above unsafe blocks.
   * Single-line regex cannot check preceding lines - flags all unsafe blocks for manual review.
   */
  rust_unsafe_block: {
    pattern: /\bunsafe\s*\{/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs', '**/benches/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'rust',
    description: 'unsafe block - verify // SAFETY: justification comment exists above'
  },

  /**
   * Rust: Hardcoded absolute path breaks portability
   * Detects paths like "/home/user/...", "/tmp/...", "/etc/..." in strings
   * Also catches raw strings: r"/home/..." and r#"/home/..."#
   */
  rust_hardcoded_path: {
    pattern: /(?:r#*)?["']\/(home|tmp|etc|usr|var|opt)\/(?!run\/)[^"']+["']/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'rust',
    description: 'Hardcoded absolute path breaks portability'
  },

  /**
   * Rust: .expect() can panic in production
   * Prefer ? operator or explicit error handling over .expect()
   */
  rust_expect_production: {
    pattern: /\.expect\s*\(\s*["']/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/tests.rs', '**/examples/**', '**/benches/**', 'build.rs', '**/build.rs', 'src/main.rs', '**/src/main.rs', 'src/bin/**', '**/src/bin/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: '.expect() can panic in production - prefer ? operator or error handling. Note: may match doc comment examples (/// lines)'
  },
  ...
  /**
   * Shell: Debug tracing left enabled
   * set -x and set -v produce verbose trace output not intended for production
   */
  shell_debugging: {
    pattern: /^\s*set\s+-[a-z]*[xv]/,
    exclude: ['*_test.sh', '**/tests/**', '*.test.sh'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'shell',
    description: 'Debug tracing (set -x/set -v) left enabled'
  },

  /**
   * Shell: Debug echo statements
   * Detects echo "DEBUG", echo "TRACE", echo "HERE" debug output
   */
  shell_echo_debug: {
    pattern: /^\s*echo\s+["']?(?:DEBUG|TRACE|HERE)\b/i,
    exclude: ['*_test.sh', '**/tests/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'shell',
    description: 'Debug echo statement left in script'
  },

  /**
   * Shell: Placeholder function with not-implemented or TODO marker
   */
  shell_placeholder_todo: {
    pattern: /^\s*(?:echo\s+["'](?:not implemented|TODO|FIXME)\b[^"']*["']|:\s+["'](?:TODO|FIXME|not implemented)\b[^"']*["'])/i,
    exclude: ['*_test.sh', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'shell',
    description: 'Placeholder not-implemented or TODO marker in shell script'
  },

  /**
   * Shell: Error silencing with || true
   * Suppresses non-zero exit codes which can mask real failures
   */
  shell_error_silencing: {
    pattern: /^\s*[^#\s].*\|\|\s*true\s*$/,
    exclude: ['*_test.sh', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'shell',
    description: 'Error silencing with || true can mask failures'
  },

  /**
   * Shell: Empty trap handler swallowing signals
   * trap '' SIGNAL discards the signal silently
   */
  shell_empty_trap: {
    pattern: /^\s*trap\s+['"]["']\s/,
    exclude: ['*_test.sh', '**/tests/**'],
    ...
    autoFix: 'flag',
    language: 'shell',
    description: 'Empty trap handler swallows signals silently'
  },

  /**
   * Shell: Hardcoded user-specific paths
   * Paths like /home/user/ or /Users/user/ break portability
   */
  shell_hardcoded_path: {
    pattern: /["']\/(?:home|Users)\/\w+\//,
    exclude: ['*_test.sh', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'shell',
    description: 'Hardcoded user-specific path breaks portability'
  },

  /**
   * Shell: Overly permissive chmod 777
   * World-writable permissions are a security risk
   */
  shell_chmod_777: {
  ...
  /**
   * Shell: Remote code execution via curl|bash or wget|sh
   * Piping remote content to shell is a major security risk
   */
  shell_curl_pipe_bash: {
    ...
    autoFix: 'flag',
    language: 'shell',
    description: 'Piping remote content to shell is a security risk'
  },

  /**
   * Shell: Unquoted variable in dangerous commands
   * Unquoted variables in rm, mv, cp etc. can cause word splitting issues
   */
  shell_unquoted_variable: {
    pattern: /\b(?:rm|mv|cp|cat|chmod|chown)\s+(?:-[a-zA-Z]+\s+)*\$[A-Za-z_]\w*(?:\s|$)/,
    exclude: ['*_test.sh', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'shell',
    description: 'Unquoted variable in dangerous command risks word splitting'
  },

  /**
   * Shell: eval command usage
   * eval is a security risk - arbitrary code execution from string
   */
  shell_eval_usage: {
    pattern: /^\s*eval\s+/,
    exclude: ['*_test.sh', '**/tests/**'],
    ...
    autoFix: 'flag',
    language: 'shell',
    description: 'eval command usage is a security risk'
  },
  ...
  // ============================================================================
  // Go patterns - debugging, error handling, and code quality
  // Discovered via false negative hunting against real-world Go codebases
  // ============================================================================

  /**
   * Go: fmt.Print/Println/Printf debugging
   * Debug print statements left in production code - use a logging package instead
   */
  go_fmt_debugging: {
    pattern: /^(?!\s*\/\/).*\bfmt\.Print(?:ln|f)?\s*\(/,
    exclude: ['*_test.go', '**/testdata/**', 'cmd/**', '**/cmd/**', 'main.go'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'go',
    description: 'Go fmt.Print() debug output in production code - use a logging package'
  },

  /**
   * Go: log.Print/Fatal/Panic debugging
   * Standard library log calls often indicate leftover debugging
   */
  go_log_debugging: {
    pattern: /^(?!\s*\/\/).*\blog\.(?:Print|Fatal|Panic)(?:ln|f)?\s*\(/,
    exclude: ['*_test.go', '**/testdata/**', 'cmd/**', '**/cmd/**', 'main.go'],
    severity: 'low',
    autoFix: 'flag',
    language: 'go',
    description: 'Go log.Print/Fatal/Panic() - consider structured logging'
  },

  /**
   * Go: spew/pp debug dumping
   * Third-party debug dump libraries left in production code
   */
  go_spew_debugging: {
    pattern: /^(?!\s*\/\/).*\b(?:spew\.(?:Dump|Printf|Sdump)|pp\.Print(?:ln)?)\s*\(/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'go',
    description: 'Go spew/pp debug dumping left in production code'
  },

  /**
   * Go: empty error check (if err != nil {})
   * Silently swallowing errors is a common AI slop pattern
   */
  go_empty_error_check: {
    ...
    autoFix: 'flag',
    language: 'go',
    description: 'Go empty error check - silently swallowing errors'
  },

  /**
   * Go: discarded error via blank identifier
   * Assigning function return to _ discards potential errors
   */
  go_discarded_error: {
    pattern: /^\s*_\s*=\s*\w+(?:\.\w+){0,10}\s*\(/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'go',
    description: 'Go discarded error via blank identifier'
  },

  /**
   * Go: bare os.Exit() in non-main code
   * os.Exit() skips deferred functions and should only be in main()
   */
  go_bare_os_exit: {
    pattern: /^(?!\s*\/\/).*\bos\.Exit\s*\(/,
    exclude: ['*_test.go', '**/testdata/**', 'cmd/**', '**/cmd/**', 'main.go'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'go',
    description: 'Go os.Exit() in non-main code - skips deferred functions'
  },

  /**
   * Go: empty interface{} parameter
   * Prefer specific types or any (Go 1.18+) over interface{}
   */
  go_empty_interface_param: {
    pattern: /func\s+\w+\s*\([^)]{0,200}\binterface\s*\{\s*\}/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'go',
    description: 'Go empty interface{} parameter - prefer specific types or any'
  },

  /**
   * Go: TODO/FIXME/HACK comment inside empty function body
   * Function with only a TODO comment indicates unfinished implementation
   */
  go_todo_empty_func: {
    pattern: /func\s+\w+\s*\([^)]{0,200}\)\s*(?:\([^)]{0,100}\)|\S+)?\s*\{\s*\/\/\s*(?:TODO|FIXME|HACK)/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'go',
    description: 'Go function with only TODO/FIXME/HACK comment - unfinished implementation'
  },

  /**
   * Go: unchecked type assertion
   * Single-value type assertion panics if type doesn't match - use comma-ok idiom
   */
  go_unchecked_type_assertion: {
    pattern: /^(?!\s*\/\/)(?!.*,\s*\w+\s*:?=).*\.\(\*?[A-Z]\w*\)/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'go',
    description: 'Go unchecked type assertion - use val, ok := x.(Type) to avoid panics'
  },

  /**
   * Go: panic for recoverable errors
   * panic() should only be used for unrecoverable programmer errors, not user input
   */
  go_panic_recoverable: {
    pattern: /^(?!\s*\/\/).*\bpanic\((?:fmt\.|errors\.|"[^"]*(?:invalid|unknown|unsupported|cannot|failed|missing|bad|wrong|no |not ))/i,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'go',
    description: 'Go panic for recoverable error - return error instead'
  },

  /**
   * Go: capitalized error string
  ...
  go_error_string_capitalized: {
    ...
    autoFix: 'flag',
    language: 'go',
    description: 'Go error string should not be capitalized (per Go conventions)'
  },

  /**
   * Go: defer Close() without error check
   * Deferred Close() can hide resource cleanup failures, especially on writes
   */
  go_defer_close_no_error: {
    pattern: /^(?!\s*\/\/).*\bdefer\s+\w+(?:\.\w+)*\.Close\(\)/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'go',
    description: 'Go defer Close() without error check - may hide cleanup failures'
  },

  /**
   * Go: weak random number generator
   * math/rand is predictable - use crypto/rand for security-sensitive operations
   */
  go_weak_random: {
    pattern: /^(?!\s*\/\/).*\brand\.New\(rand\.NewSource\(/,
    exclude: ['*_test.go', '**/testdata/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'go',
    description: 'Go weak random (math/rand) - use crypto/rand for security-sensitive ops'
  },

  /**
   * Go: unused append result
   * append() returns a new slice - discarding it is always a bug
   */
  go_unused_append: {
    pattern: /^\s*append\(/,
    exclude: ['*_test.go', '**/testdata/**'],
    ...
    autoFix: 'flag',
    language: 'go',
    description: 'Go append() result not assigned - this is always a bug'
  },
  ...
  /**
   * Java: System.out.println() / System.err.println() debugging
   * Common AI-generated debug output left in production code
   */
  java_sysout_debugging: {
    pattern: /System\.(out|err)\.(print(?:ln|f)?|format|write|append)\s*\(/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**', '**/examples/**', '**/demo/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'java',
    description: 'Java System.out/err debug output in production code'
  },

  /**
   * Java: e.printStackTrace() debugging
   * Should use proper logging framework instead
   */
  java_stacktrace_debugging: {
    pattern: /\.printStackTrace\s*\(\s*\)/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'java',
    description: 'Java printStackTrace() - use logging framework instead'
  },

  /**
   * Java: throw new RuntimeException("TODO") / IllegalStateException("not implemented")
   * Placeholder exceptions indicating unfinished implementation
   */
  java_throw_todo: {
    pattern: /throw\s+new\s+(?:RuntimeException|IllegalStateException|IllegalArgumentException)\s*\(\s*["'].{0,200}(?:TODO|not\s+impl|implement|FIXME|HACK)/i,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'java',
    description: 'Java placeholder exception (TODO/not implemented)'
  },

  /**
   * Java: return null; // TODO
   * Placeholder return indicating unfinished logic
   */
  java_return_null_todo: {
    pattern: /return\s+null\s*;\s*\/\/\s*(?:TODO|FIXME|HACK|implement|placeholder|stub)/i,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'java',
    description: 'Java return null with TODO/placeholder comment'
  },

  /**
   * Java: catch (Exception e) { } - empty catch block
   * Silently swallows errors - should at minimum log the exception
   */
  java_empty_catch: {
    pattern: /catch\s*\([^)]*\)\s*\{\s*\}/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    ...
    autoFix: 'add_logging',
    language: 'java',
    description: 'Java empty catch block - silently swallows errors'
  },

  /**
   * Java: catch (Exception e) { // ignore } - intentionally ignored error
   * Often used as a quick fix but masks real problems
   */
  java_catch_ignore: {
    pattern: /catch\s*\([^)]*\)\s*\{\s*\/\/\s*(?:ignore|suppress|noop|nothing|empty|intentional)/i,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'java',
    description: 'Java catch block that intentionally ignores errors'
  },

  /**
   * Java: @SuppressWarnings("unchecked") etc.
   * May hide legitimate type safety warnings
   */
  java_suppress_warnings: {
    pattern: /@SuppressWarnings\s*\(\s*(?:["'](?:unchecked|rawtypes|all|deprecation)["']|\{[^}]*["'](?:unchecked|rawtypes|all|deprecation)["']|value\s*=\s*["'](?:unchecked|rawtypes|all|deprecation)["'])/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'java',
    description: 'Java @SuppressWarnings hiding type safety or deprecation warnings'
  },

  /**
   * Java: raw generics usage (List items = instead of List<String> items =)
   * Indicates unsafe type erasure reliance
   */
  java_raw_type: {
    pattern: /\b(?:List|Map|Set|Collection|Iterable|Iterator|Queue|Deque|HashMap|ArrayList|LinkedList|TreeMap|TreeSet|HashSet|LinkedHashMap|LinkedHashSet|ConcurrentHashMap|Vector|Stack|Hashtable)\s+\w+\s*[=;]/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'java',
    description: 'Java raw generic type - missing type parameter'
  },

  /**
   * Java: catch (Exception e) - overly broad exception catch
   * Should catch specific exceptions for proper error handling
   */
  java_wildcard_catch: {
    pattern: /catch\s*\(\s*(?:Exception|Throwable|Error)\s+\w+\s*\)/,
    exclude: ['*Test.java', '**/test/**', '**/tests/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'java',
    description: 'Java overly broad exception catch - prefer specific exception types'
  },

  // ─── Kotlin patterns ─────────────────────────────────────────────────

  /**
   * Kotlin: println() debugging
   * Kotlin's top-level println() is commonly left in code by AI generators
   */
  kotlin_println_debugging: {
    pattern: /(?<!\w)print(?:ln)?\s*\(/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**', '**/examples/**', '**/demo/**', '**/snippets/**', 'main.kt', 'Main.kt', '*.gradle.kts', '*.gradle'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'kotlin',
    description: 'Kotlin print/println() debug output in production code'
  },

  /**
   * Kotlin: TODO() stdlib call
   * Throws NotImplementedError at runtime - placeholder code
   */
  kotlin_todo_call: {
    pattern: /(?<!\w)TODO\s*\(/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'kotlin',
    description: 'Kotlin TODO() call - throws NotImplementedError at runtime'
  },

  /**
   * Kotlin: // FIXME with placeholder code
  ...
  kotlin_fixme_comment: {
    ...
    autoFix: 'flag',
    language: 'kotlin',
    description: 'Kotlin FIXME comment - indicates unfinished implementation'
  },

  /**
   * Kotlin: empty catch block
   * catch (e: Exception) { } silently swallows errors
   */
  kotlin_empty_catch: {
    pattern: /catch\s*\([^)]*\)\s*\{\s*\}/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**'],
    ...
    autoFix: 'add_logging',
    language: 'kotlin',
    description: 'Kotlin empty catch block - silently swallows errors'
  },

  /**
   * Kotlin: runCatching{}.getOrNull() - swallowed error
   * Silently discards the error from runCatching
   */
  kotlin_swallowed_error: {
    pattern: /runCatching\s*\{[^}]*\}\s*\.\s*(?:getOrNull\s*\(\s*\)|getOrDefault\s*\(\s*null\s*\)|getOrElse\s*\{\s*null\s*\})/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'kotlin',
    description: 'Kotlin runCatching error silently swallowed (getOrNull/getOrDefault(null)/getOrElse{null})'
  },

  /**
   * Kotlin: @Suppress("UNCHECKED_CAST") etc.
   * May hide legitimate compiler warnings
   */
  kotlin_suppress_annotation: {
    pattern: /(?:@file:)?@?Suppress\s*\(\s*(?:["'](?:UNCHECKED_CAST|DEPRECATION|unused)["']|["'][^"']*["']\s*,\s*["'](?:UNCHECKED_CAST|DEPRECATION|unused)["'])/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'kotlin',
    description: 'Kotlin @Suppress hiding type safety or deprecation warnings'
  },

  /**
   * Kotlin: Android Log.d/v/i debug logging
   * Debug and verbose logging left in production code
   */
  kotlin_android_debug_log: {
    pattern: /\bLog\.(d|v|i)\s*\(/,
    exclude: ['*Test.kt', '**/test/**', '**/tests/**', '**/androidTest/**', '**/debug/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'kotlin',
    description: 'Android Log.d/v/i debug logging in production code'
  },
   ...
   * Detects module-level mutable lists assigned to UPPERCASE names
   * Note: UPPERCASE dicts/sets excluded - standard Python constant pattern
   */
  mutable_globals_py: {
    pattern: /^[A-Z][A-Z0-9_]*\s*=\s*(?:\[|list\()/m,
    exclude: ['test_*.py', '*_test.py', 'conftest.py', '**/tests/**', 'constants.py', 'settings.py', 'config.py', 'defaults.py'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'python',
    description: 'Mutable global list - consider tuple or frozenset for immutability'
  },

  /**
   * Python: Bare except clause without exception type
   * Catches all exceptions including SystemExit, KeyboardInterrupt - almost always a bug
   */
  python_bare_except: {
    ...
    autoFix: 'flag',
    language: 'python',
    description: 'Bare except: without exception type - catches SystemExit/KeyboardInterrupt'
  },

  /**
   * Python: eval()/exec() usage
   * Arbitrary code execution - security risk in production code
   */
  python_eval_exec: {
    ...
    autoFix: 'flag',
    language: 'python',
    description: 'eval()/exec() usage - arbitrary code execution risk'
  },

  /**
   * Python: os.system() calls
   * Prefer subprocess module for better security and error handling
   */
  python_os_system: {
    ...
    autoFix: 'flag',
    language: 'python',
    description: 'os.system() call - use subprocess module instead'
  },

  /**
   * Python: Overly permissive file permissions (chmod 777)
   * World-writable files are a security risk
   */
  python_chmod_777: {
    pattern: /\bos\.chmod\s*\([^,)]+,\s*0o777\s*\)/,
    exclude: ['test_*.py', '*_test.py', 'conftest.py', '**/tests/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'python',
    description: 'os.chmod with 0o777 - overly permissive file mode'
  },

  /**
   * Python: Hardcoded user home paths
   * Use os.path.expanduser("~") or pathlib.Path.home() instead
   */
  python_hardcoded_path: {
    ...
    autoFix: 'flag',
    language: 'python',
    description: 'Hardcoded user home path - use os.path.expanduser("~") or Path.home()'
  },

  /**
   * Python: logging.basicConfig with DEBUG level
   * DEBUG level in production leaks verbose internal state
   */
  python_logging_debug: {
    pattern: /logging\.basicConfig\s*\([\s\S]*?level\s*=\s*logging\.DEBUG/,
    exclude: ['test_*.py', '*_test.py', 'conftest.py', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'python',
    description: 'logging.basicConfig with DEBUG level - too verbose for production'
  },

  /**
   * Python: Debug prints of os.environ or sys.argv
   * Leaks environment variables and command-line arguments
   */
  python_os_environ_debug: {
    pattern: /\bprint\s*\(\s*(?:os\.environ|sys\.argv)/,
    exclude: ['test_*.py', '*_test.py', 'conftest.py', '**/tests/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'python',
    description: 'Debug print of os.environ/sys.argv - leaks sensitive information'
  },

  /**
   * Python: subprocess with shell=True
   * Shell injection risk when combined with user input
   */
  ...
  // ============================================================================
  // C patterns (also match C++ files via pipeline cross-language matching)
  // ============================================================================

  /**
   * C: printf/fprintf debugging with DEBUG/TRACE/HERE markers
   * Catches leftover debug prints - not regular output statements
   */
  c_printf_debugging: {
    pattern: /\b(?:printf|fprintf)\s*\(\s*(?:stderr\s*,\s*)?["'](?:DEBUG|TRACE|HERE)\b/i,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'c',
    description: 'Debug printf/fprintf statement left in production code'
  },

  /**
   * C: #ifdef DEBUG or #if 0 blocks
   * Debug-only blocks or disabled code left in source
   */
  c_ifdef_debug_block: {
    pattern: /^\s*#\s*(?:ifdef\s+DEBUG|if\s+0)\b/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'c',
    description: '#ifdef DEBUG or #if 0 block left in source'
  },

  /**
   * C: assert(false/0 && "not implemented/TODO")
   * Placeholder assertion that crashes at runtime
   */
  c_placeholder_todo: {
    pattern: /\bassert\s*\(\s*(?:false|0)\s*&&\s*["'][^"']{0,200}(?:not\s+impl|TODO|implement)/i,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'c',
    description: 'Placeholder assert(false && "TODO") crashes at runtime'
  },

  /**
   * C: #pragma warning disable / diagnostic ignored
   * Warning suppression that may hide real issues
   */
  c_pragma_warning_disable: {
    pattern: /^\s*#\s*pragma\s+(?:warning\s*\(\s*disable|GCC\s+diagnostic\s+ignored|clang\s+diagnostic\s+ignored)/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**', 'vendor/**', '**/vendor/**', 'third_party/**', '**/third_party/**', 'third-party/**', '**/third-party/**', 'external/**', '**/external/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'c',
    description: 'Warning suppression pragma may hide real issues'
  },

  /**
   * C: goto usage
   * DISABLED: goto is idiomatic C for error/resource cleanup. Tested against
   * libuv, valkey, criu, bcc - virtually all goto usage is legitimate.
   * Negative lookahead approach cannot enumerate all valid cleanup labels.
   */
  c_goto_usage: {
    pattern: null,  // DISABLED - goto is idiomatic C, not slop
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'c',
    description: 'goto detection disabled (idiomatic C cleanup pattern)'
  },

  /**
   * C: Hardcoded paths to sensitive credential files
   * References to /etc/shadow, SSH keys, etc. should not be hardcoded
   */
  c_hardcoded_credential_path: {
    pattern: /["']\/(?:etc\/(?:shadow|passwd|ssl\/private)|home\/\w+\/\.ssh\/(?:id_rsa|id_ed25519|authorized_keys))["']/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'critical',
    autoFix: 'flag',
    language: 'c',
    description: 'Hardcoded path to sensitive credential file'
  },

  /**
   * C: Magic number cast to pointer type
   * Casting hex literals to pointer types is fragile and platform-specific
   */
  c_magic_number_cast: {
    pattern: /\(\s*(?:void|char|int|unsigned|long)\s*\*\s*\)\s*0x[0-9a-fA-F]{4,}/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'c',
    description: 'Magic number cast to pointer type is fragile and platform-specific'
  },
  ...
  /**
   * C++: std::cout/cerr debugging output
   * Flags cout/cerr usage in library code - should use a logging framework
   */
  cpp_cout_debugging: {
    pattern: /\bstd::(?:cout|cerr)\s*<</,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**', '**/examples/**', '**/main.cpp', '**/main.cc'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'cpp',
    description: 'std::cout/cerr in library code - use a logging framework instead'
  },

  /**
   * C++: throw not-implemented exception
   * Placeholder exception that indicates unfinished code
   */
  cpp_throw_not_implemented: {
    pattern: /\bthrow\s+(?:std::)?(?:runtime_error|logic_error|domain_error)\s*\(\s*["'][^"']{0,200}(?:not\s+impl|TODO|implement)/i,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'cpp',
    description: 'throw not-implemented exception - placeholder code'
  },

  /**
   * C++: Empty catch block swallows exceptions silently
   */
  cpp_empty_catch: {
    pattern: /catch\s*\([^)]*\)\s*\{\s*\}/,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**'],
  ...
  // ============================================================================
  // C patterns - unsafe API usage and hardcoded values
  // Discovered via false negative hunting against libuv, valkey, criu, bcc
  // ============================================================================

  /**
   * C: sprintf() without bounds checking
   * Prefer snprintf() which takes a buffer size argument
   */
  c_sprintf_usage: {
    pattern: /\bsprintf\s*\(/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**', 'vendor/**', '**/vendor/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'c',
    description: 'sprintf() without bounds checking - prefer snprintf()'
  },

  /**
   * C: strcpy() without bounds checking
   * Prefer strncpy() or strlcpy() which take a buffer size argument
   */
  c_strcpy_usage: {
    pattern: /\bstrcpy\s*\(/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**', 'vendor/**', '**/vendor/**'],
    ...
    autoFix: 'flag',
    language: 'c',
    description: 'strcpy() without bounds checking - prefer strncpy() or strlcpy()'
  },

  /**
   * C: atoi/atol/atof with no error handling
   * These functions return 0 on failure with no way to distinguish from valid 0.
   * Prefer strtol()/strtod() which provide error detection.
   */
  c_unsafe_atoi: {
    ...
    autoFix: 'flag',
    language: 'c',
    description: 'atoi()/atol()/atof() has no error handling - prefer strtol()/strtod()'
  },

  /**
   * C: Hardcoded IP addresses or localhost
   * Should use configuration rather than embedding network addresses
   */
  c_hardcoded_ip: {
    pattern: /["'](?:127\.0\.0\.1|0\.0\.0\.0|localhost)["']/,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**', '**/examples/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'c',
    description: 'Hardcoded IP address or localhost - use configuration instead'
  },

  /**
   * C: Hardcoded /tmp debug file paths
   * Should use proper temp directory API (mkstemp, tmpnam, etc.)
   */
  c_hardcoded_debug_path: {
    ...
    autoFix: 'flag',
    language: 'c',
    description: 'Hardcoded /tmp file path - use proper temp directory API'
  },

  /**
   * C: Return/init values only to suppress compiler warnings
   * Code that assigns values solely to silence warnings should be refactored
   */
  c_return_avoid_warning: {
    pattern: /(?:return\s+(?:-?\d+|NULL)\s*;|=\s*\d+\s*;)\s*\/\*.{0,100}(?:avoid|suppress|silence).{0,50}warn/i,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'c',
    description: 'Value assigned only to suppress compiler warning - refactor to eliminate'
  },

  /**
   * C: Debug fprintf/printf behind runtime debug flags
   * Debug prints gated by runtime flags left in production code
   */
  c_debug_fprintf_conditional: {
    pattern: /if\s*\(\s*\w*(?:debug|dbg|verbose|trace)\w*\s*\)\s*(?:\{?\s*)?(?:fprintf|printf)\s*\(/i,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'c',
    description: 'Debug print behind runtime flag left in production code'
  },

  /**
   * C: Unchecked malloc/calloc/realloc return value
   * MULTI-PASS: Requires analysis of surrounding lines to check for NULL guards.
   * Single-line regex cannot reliably detect this pattern.
   */
  c_unchecked_malloc: {
    pattern: null,
    requiresMultiPass: true,
    ...
    autoFix: 'flag',
    language: 'c',
    description: 'Unchecked malloc/calloc return value (requires multi-pass analysis)'
  },

  /**
   * C: TODO/FIXME with stub or empty implementation
   * MULTI-PASS: Requires analysis of function body to detect stubs.
   * Single-line regex cannot reliably detect this pattern.
   */
  c_todo_stub_function: {
    pattern: null,
    requiresMultiPass: true,
    exclude: ['*_test.c', '*_test.cpp', '**/tests/**', '**/test/**'],
    severity: 'high',
    autoFix: 'flag',
    language: 'c',
    description: 'TODO/FIXME comment with stub or empty implementation (requires multi-pass analysis)'
  },

  // ============================================================================
  // C++ patterns - modern C++ idiom violations
  // Discovered via false negative hunting against real-world C++ codebases
  // ============================================================================

  /**
   * C++: Raw new/delete without smart pointers
   * Modern C++ should use std::unique_ptr or std::shared_ptr
   */
  cpp_raw_new_delete: {
    pattern: /\b(?:new\s+\w[\w:]*\s*[\[(]|delete\s+\w)/,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'cpp',
    description: 'Raw new/delete - prefer std::unique_ptr or std::shared_ptr'
  },

  /**
   * C++: C-style pointer casts
   * Modern C++ should use static_cast/reinterpret_cast for type safety
   */
  cpp_c_style_cast: {
    pattern: /\(\s*(?:const\s+)?(?:struct\s+)?(?:unsigned\s+)?(?:char|int|void|long|short|float|double|uint\d+_t|int\d+_t)\s*\*\s*\)/,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'cpp',
    description: 'C-style pointer cast in C++ - prefer static_cast/reinterpret_cast'
  },

  /**
   * C++: fprintf(stderr) instead of proper error handling
   * C++ code should use std::cerr or a logging framework
   */
  cpp_fprintf_stderr: {
    pattern: /fprintf\s*\(\s*stderr\s*,/,
    exclude: ['*_test.cpp', '*_test.cc', '**/tests/**', '**/test/**', 'deps/**', '**/deps/**', '**/examples/**', '**/main.cpp', '**/main.cc'],
    severity: 'low',
    autoFix: 'flag',
    language: 'cpp',
    description: 'fprintf(stderr) in C++ - use std::cerr or logging framework'
  }
};
