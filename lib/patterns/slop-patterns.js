...
const slopPatterns = {
   ...
   */
  rust_log_debug: {
    pattern: /\blog::(debug|trace)!\s*\(/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**'],
    severity: 'medium',
    autoFix: 'remove',
    language: 'rust',
    description: 'log::debug!()/trace!() left in production code'
  },
  ...
  rust_unnecessary_clone: {
    pattern: /\.clone\(\s*\)/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/benches/**'],
    severity: 'low',
    autoFix: 'flag',
    language: 'rust',
    description: 'Potential unnecessary .clone() call - review if clone is needed'
  },
   ...
   */
  rust_hardcoded_path: {
    pattern: /(?:r#*)?["']\/(home|tmp|etc|usr|var|opt)\/[^"']+["']/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'rust',
  ...
  rust_expect_production: {
    pattern: /\.expect\s*\(\s*["']/,
    exclude: ['*_test.rs', '*_tests.rs', '**/tests/**', '**/examples/**', '**/benches/**', 'build.rs'],
    severity: 'medium',
    autoFix: 'flag',
    language: 'rust',
    description: '.expect() can panic in production - use ? operator or handle error'
  },
