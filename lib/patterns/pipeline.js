...
const { getAiTargetFiles, getTestGaps, getRiskScores } = require('../collectors/codebase');
...
async function runPipeline(repoPath, options = {}) {
  ...
  // Risk weighting from repo-intel (test-gaps + diff-risk)
  const testGaps = getTestGaps(repoPath);
  const riskScores = getRiskScores(repoPath, targetFiles);

  if (testGaps || riskScores) {
    for (const finding of findings) {
      // Escalate MEDIUM findings in untested files to HIGH
      if (testGaps && testGaps.has(finding.file) && finding.certainty === 'MEDIUM') {
        finding.certainty = 'HIGH';
        finding.message = (finding.message || '') + ' (escalated: no test coupling)';
      }
    ...
    // Re-sort: certainty first, then risk score within tier
