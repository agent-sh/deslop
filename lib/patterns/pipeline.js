...
const { getAiTargetFiles } = require('../collectors/codebase');
...
async function runPipeline(repoPath, options = {}) {
  ...
  if (!targetFiles || targetFiles.length === 0) {
    // Try AI-targeted files first (prioritizes files with recent AI-authored changes)
    const aiFiles = getAiTargetFiles(repoPath, { limit: maxFiles });
    ...
    for (const [patternName, pattern] of Object.entries(patterns)) {
      ...
      if (pattern.language) {
        ...
        const langMatch = (patternLang === 'javascript' && fileLanguage === 'js') ||
                          ...
                          (patternLang === 'kotlin' && fileLanguage === 'kotlin') ||
                          (patternLang === 'shell' && fileLanguage === 'shell') ||
                          (patternLang === 'c' && fileLanguage === 'c') ||
                          (patternLang === 'c' && fileLanguage === 'cpp') ||  // C patterns also fire on C++ files
                          (patternLang === 'cpp' && fileLanguage === 'cpp') ||
...
async function runMultiPassAnalyzers(repoPath, targetFiles, fileContents) {
  ...
  const docCodeLangs = /\.(js|jsx|ts|tsx|mjs|cjs|py|rs|java|kt|kts|go|c|h|cpp|cc|cxx|hpp|hxx)$/i;
  ...
  if (stubPattern) {
    ...
    const stubExtensions = /\.(js|jsx|ts|tsx|mjs|cjs|py|rs|java|go|c|h|cpp|cc|cxx|hpp|hxx)$/i;
...
function runPhase2(repoPath, cliTools, targetFiles) {
  ...
  // Go lint analysis with golangci-lint
  if (cliTools.golangci_lint) {
    const golangciResults = cliEnhancers.runGolangciLint(repoPath, targetFiles);
    if (golangciResults) {
      for (const result of golangciResults) {
        findings.push({
          file: result.file,
          line: result.line || 0,
          patternName: 'golangci_lint_issue',
          severity: result.severity || 'medium',
          certainty: CERTAINTY.LOW,
          description: `golangci-lint (${result.linter}): ${result.message}`,
          autoFix: 'flag',
