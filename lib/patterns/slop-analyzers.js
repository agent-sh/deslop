...
function getDocCodeConfig(lang) {
  const configs = {
    ...
    c: {
      ...
      docPatterns: [
        /\/\*\*([\s\S]*?)\*\/\s*(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:\w+[\s*]+)+(\w+)\s*\(/g,
    ...
    cpp: {
      ...
      docPatterns: [
        /\/\*\*([\s\S]*?)\*\/\s*(?:(?:virtual|static|inline|explicit|constexpr|const|override)\s+)*(?:\w+(?:::\w+)*[\s*&]+)+(\w+)\s*\(/g,
        /((?:^\s*\/\/\/.*\n)+)\s*(?:(?:virtual|static|inline|explicit|constexpr|const|override)\s+)*(?:\w+(?:::\w+)*[\s*&]+)+(\w+)\s*\(/gm,
      ],
    },
    python: {
      useBraces: false,
...
const EXPORT_PATTERNS = {
  ...
  c: [
    /^(?!static\s)(?:extern\s+)?(?:const\s+)?(?:\w+[\s*]+)+\w+\s*\([^)]*\)\s*;/gm,  // Non-static function declarations in headers
    /^(?!static\s)(?:const\s+)?(?:\w+[\s*]+)+\w+\s*\([^)]*\)\s*\{/gm               // Non-static function definitions
  ],
  cpp: [
    /^(?:class|struct)\s+\w+/gm,
    /^(?:namespace)\s+\w+/gm,
    /^(?!static\s)(?:(?:virtual|inline|explicit|constexpr)\s+)*(?:\w+(?:::\w+)*[\s*&]+)+\w+\s*\(/gm
  ]
};
...
function getStubFunctionConfig(lang) {
  const configs = {
    ...
    c: {
      useBraces: true,
      functionPatterns: [
        /(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:\w+[\s*]+)+(\w+)\s*\([^)]*\)\s*\{/g,
      ],
    ...
    cpp: {
      useBraces: true,
      functionPatterns: [
        /(?:(?:virtual|static|inline|explicit|constexpr)\s+)*(?:\w+(?:::\w+)*[\s*&]+)+(\w+)\s*\([^)]*\)(?:\s*(?:const|override|noexcept))?\s*\{/g,
      ],
