...
function getDocCodeConfig(lang) {
  const configs = {
    ...
    kotlin: {
      useBraces: true,
      // KDoc: /** ... */ followed by fun/class/object/val
      docPatterns: [
        /\/\*\*([\s\S]*?)\*\/\s*(?:@\w+\s*)*(?:(?:public|private|protected|internal)\s+)?(?:(?:suspend|inline|open|abstract|override|data|sealed)\s+)*(?:fun|class|object|val|var)\s+(\w+)/g,
      ],
    },
    rust: {
      useBraces: true,
    ...
    c: {
      useBraces: true,
      // Doxygen: /** ... */ or /// lines before function
      docPatterns: [
        /\/\*\*([\s\S]*?)\*\/\s*(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:\w+[\s*]+){1,5}(\w+)\s*\(/g,
        /((?:^\s*\/\/\/.*\n)+)\s*(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:\w+[\s*]+){1,5}(\w+)\s*\(/gm,
      ],
    },
    cpp: {
      useBraces: true,
      // Doxygen: /** ... */ or /// lines before function/method
      docPatterns: [
        /\/\*\*([\s\S]*?)\*\/\s*(?:(?:virtual|static|inline|explicit|constexpr|const|override)\s+){0,5}(?:\w+(?:::\w+){0,5}[\s*&]+){1,5}(\w+)\s*\(/g,
        /((?:^\s*\/\/\/.*\n)+)\s*(?:(?:virtual|static|inline|explicit|constexpr|const|override)\s+){0,5}(?:\w+(?:::\w+){0,5}[\s*&]+){1,5}(\w+)\s*\(/gm,
      ],
    },
    python: {
      useBraces: false,
...
const COMMENT_SYNTAX = {
  ...
  java: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\//, len: 2 } },
  kotlin: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\//, len: 2 } },
  shell: { line: /^\s*#/, block: null },
  c: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\//, len: 2 } },
  cpp: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\//, len: 2 } }
};
...
const ENTRY_POINTS = [
  ...
  'Application.kt', 'src/main/kotlin/Application.kt',
  'App.kt', 'src/main/kotlin/App.kt',
  'main.c', 'src/main.c',
...
const EXPORT_PATTERNS = {
  ...
  kotlin: [
    /^\s*(?:public\s+)?fun\s+\w+/gm,
    /^\s*(?:public\s+)?(?:data\s+|sealed\s+|abstract\s+|open\s+)?class\s+\w+/gm,
    /^\s*(?:public\s+)?object\s+\w+/gm,
    /^\s*(?:public\s+)?val\s+\w+/gm
  ],
  c: [
    /^(?!static\s)(?:extern\s+)?(?:const\s+)?(?:\w+[\s*]+){1,5}\w+\s*\([^)]*\)\s*;/gm,  // Non-static function declarations in headers
    /^(?!static\s)(?:const\s+)?(?:\w+[\s*]+){1,5}\w+\s*\([^)]*\)\s*\{/gm               // Non-static function definitions
  ],
  cpp: [
    /^(?:class|struct)\s+\w+/gm,
    /^(?:namespace)\s+\w+/gm,
    /^(?!static\s)(?:(?:virtual|inline|explicit|constexpr)\s+){0,5}(?:\w+(?:::\w+){0,5}[\s*&]+){1,5}\w+\s*\(/gm
  ]
};
...
/**
 * Detect language from file extension, with optional shebang detection
 * @param {string} filePath - Path to file
 * @param {string} [content] - File content for shebang detection on extensionless files
...
function detectLanguage(filePath, content) {
  ...
  if (content) {
    ...
    if (/^#!.*\bpython[23]?(?:\s|$)/.test(firstLine)) return 'python';
  }
  ...
  try {
    ...
    for (const line of logResult.split('\n')) {
      ...
      if (trimmed.startsWith('COMMIT:')) {
        ...
        if (['.js', '.ts', '.jsx', '.tsx', '.py', '.go', '.rs', '.java', '.kt', '.kts'].includes(ext)) {
...
function getStubFunctionConfig(lang) {
  const configs = {
    ...
    c: {
      useBraces: true,
      functionPatterns: [
        /(?:static\s+)?(?:inline\s+)?(?:const\s+)?(?:\w+[\s*]+){1,5}(\w+)\s*\([^)]*\)\s*\{/g,
      ],
      stubReturnPattern: /^\s*return\s+(0|NULL|-1|1|0\.0|0\.0f)\s*;\s*$/,
      commentPatterns: [/^\s*\/\//, /^\s*\/\*/, /^\s*\*/, /^\s*\*\/$/],
    },
    cpp: {
      useBraces: true,
      functionPatterns: [
        /(?:(?:virtual|static|inline|explicit|constexpr)\s+){0,5}(?:\w+(?:::\w+){0,5}[\s*&]+){1,5}(\w+)\s*\([^)]*\)(?:\s*(?:const|override|noexcept))?\s*\{/g,
      ],
      stubReturnPattern: /^\s*return\s+(0|nullptr|NULL|-1|true|false|std::string\(\)|"")\s*;\s*$/,
      throwStubPattern: /^\s*throw\s+(?:std::)?(?:runtime_error|logic_error)\s*\([^)]*\)\s*;\s*$/,
      commentPatterns: [/^\s*\/\//, /^\s*\/\*/, /^\s*\*/, /^\s*\*\/$/],
    },
    python: {
      useBraces: false,
...
/**
 * Analyze brace-delimited languages (JS, Rust, Java, Go, C, C++)
 */
