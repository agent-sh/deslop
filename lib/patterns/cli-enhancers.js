 ...
 *
 * Supported languages: javascript, typescript, python, rust, go, java, kotlin, shell, c, cpp
 *
 ...
 */
const SUPPORTED_LANGUAGES = ['javascript', 'typescript', 'python', 'rust', 'go', 'java', 'kotlin', 'shell', 'c', 'cpp'];
...
const CLI_TOOLS = {
  ...
  jscpd: {
    ...
    languages: ['javascript', 'typescript', 'python', 'go', 'rust', 'java', 'kotlin', 'c', 'cpp']
  },
  ...
  cppcheck: {
    name: 'cppcheck',
    description: 'Static analysis for C/C++ code defects',
    checkCommand: 'cppcheck --version',
    installHint: 'apt install cppcheck / brew install cppcheck / choco install cppcheck',
    languages: ['c', 'cpp']
  },
  clang_tidy: {
    name: 'clang-tidy',
    description: 'C/C++ linter based on Clang with modernize checks',
    checkCommand: 'clang-tidy --version',
    installHint: 'apt install clang-tidy / brew install llvm / choco install llvm',
    languages: ['c', 'cpp']
  }
};
...
/**
 * Run Go lint analysis using golangci-lint
 *
 * @param {string} repoPath - Repository root path
 * @param {string[]} targetFiles - Files to analyze (used for filtering results)
 * @param {Object} options - Options
 * @returns {Array|null} Lint findings, or null if tool not available
 */
function runGolangciLint(repoPath, targetFiles, options = {}) {
  if (!isToolAvailable(CLI_TOOLS.golangci_lint.checkCommand)) {
    return null;
  }

  try {
    // Run golangci-lint with JSON output, zero exit code on issues
    const args = [
      'run',
      '--out-format=json',
      '--issues-exit-code=0',
    ...
    const result = execFileSync('golangci-lint', args, {
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 120000,
      windowsHide: true,
      cwd: repoPath,
    ...
    try {
      ...
      if (report.Issues) {
        for (const issue of report.Issues) {
          // Map golangci-lint severity to deslop severity
          let severity = 'medium';
          ...
          findings.push({
            ...
            message: issue.Text || '',
            linter: issue.FromLinter || 'unknown',
            severity
          });
        }
      }

      // Filter to target files when specified
      if (targetFiles && targetFiles.length > 0) {
        return findings.filter(f =>
          targetFiles.some(tf => tf.endsWith(f.file) || f.file.endsWith(tf))
        );
      }

      return findings;
    } catch {
      // JSON parsing failed - invalid output from tool
      return [];
    }
  } catch {
    // Tool execution failed or not available
    return null;
  }
}
